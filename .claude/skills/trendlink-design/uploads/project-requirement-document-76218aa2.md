...
flowchart TD
    ...
    A --> T[負責人：工作任務設定<br/>設定值日生工作任務（備註）]
...
1. **前置設定**：負責人先在「活動設定」標好國定假日、調整上班日，並在「工作任務設定」維護值日生工作任務（以備註形式顯示在班表上）；值日生各自在「排除時段」標記不能排的日期。
...
- ##### 目標：提供[排班負責人](#排班負責人)設定每個月的工作任務，以利在[正式班表](#正式班表)上顯示每個月的工作任務備註內容
...
- #### 規格
  - 工作任務設定頁面
    - 提供[排班負責人](#排班負責人)設定[值日生](#值日生)的工作任務，並以備註形式顯示在班表上，以利各[值日生](#值日生)清楚自己要完成的工作內容
    - 工作任務設定會套用所有[值日生](#值日生)，讓[排班負責人](#排班負責人)不需要針對每個[值日生](#值日生)分別設定工作任務，提升設定效率
    - 工作任務設定可由[排班負責人](#排班負責人)隨時新增、修改與刪除
    - 工作任務設定頁面提供一個文字輸入框，讓[排班負責人](#排班負責人)可以輸入[值日生](#值日生)需要完成的工作內容，並且這些內容會以備註形式顯示在[正式班表](#正式班表)上。字數限制為 200 字以內，確保備註內容簡潔明瞭，方便[值日生](#值日生)快速了解自己的工作任務
    - 工作任務設定頁面提供保存功能，讓[排班負責人](#排班負責人)在設定完工作任務後，可以保存這些資料，並且這些資料能夠正確存儲在系統中並且呈現在當前介面
    - 沒有工作任務時，需要有 Empty State 的提示，讓[排班負責人](#排班負責人)可以清楚知道目前沒有設定工作任務

- ##### 卡控機制
  - 工作任務設定的備註內容為必填，且字數限制為 200 字以內，不符合條件將無法保存（保存按鈕狀態 Disabled）

- ##### 驗收標準

| Scenario                                            | Given                                                                      | When                                                                      | Then                                                                                                                                            |
| --------------------------------------------------- | -------------------------------------------------------------------------- | ------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| 成功設定工作任務並顯示在[正式班表](#正式班表)上     | [排班負責人](#排班負責人)進入工作任務設定頁面                              | [排班負責人](#排班負責人)在文字輸入框中輸入工作任務內容，並且點擊保存按鈕 | 成功保存工作任務設定，並且這些資料能夠正確存儲在系統中並且呈現在當前介面；同時，在[正式班表](#正式班表)上，顯示該工作任務的備註內容             |
| 成功調整工作任務設定並更新在[正式班表](#正式班表)上 | [排班負責人](#排班負責人)已經設定了工作任務並顯示在[正式班表](#正式班表)上 | [排班負責人](#排班負責人)在文字輸入框中修改工作任務內容，並且點擊保存按鈕 | 成功保存修改後的工作任務設定，並且這些資料能夠正確存儲在系統中並且呈現在當前介面；同時，在[正式班表](#正式班表)上，顯示更新後的工作任務備註內容 |
| 成功刪除工作任務設定並更新在[正式班表](#正式班表)上 | [排班負責人](#排班負責人)已經設定了工作任務並顯示在[正式班表](#正式班表)上 | [排班負責人](#排班負責人) 點擊刪除按鈕並保存                              | 成功刪除工作任務設定，並且這些變更能夠正確存儲在系統中並且呈現在當前介面；同時，在[正式班表](#正式班表)上，不再顯示該工作任務的備註內容         |
...
#### Phase 4 — 正式班表 + 工作任務設定 + 出勤統計

**目標：班表檢視與數據回饋**

- 正式班表頁面（日曆檢視、篩選值日生、編輯模式、無法出勤標記）
- 工作任務設定頁面（工作任務 CRUD，備註顯示於正式班表）
- 出勤統計頁面（年度表格、排班平均次數/實際排班次數/缺勤次數、日曆聯動）

**理由：** 正式班表是草稿發布後的產物，出勤統計又依賴正式班表資料，工作任務以備註形式顯示在正式班表上，三者放在一起順理成章。
...
gantt
    ...
    Phase 4 — 正式班表 + 工作任務 + 出勤統計 :p4, after p3, 3d
...
| **Phase 4**   | 3d   | 06/27 – 07/01 | 正式班表、工作任務設定、出勤統計 |
| **Phase 5**   | 1d   | 07/02         | 過往匯入、Docker 部署           |
